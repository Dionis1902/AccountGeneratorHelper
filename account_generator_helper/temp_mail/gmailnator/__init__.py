...
class GmailNator(Mail):
    """
    Class for work with https://emailnator.com/
    """

    def __init__(self, proxy=None):
        super().__init__(proxy)
        r = self._s.get("https://emailnator.com/")
        ...
        payload = json.dumps(
            {
                "email": [
                    i
                    for i, k in zip(data, [use_custom_domain, use_plus, use_point])
                    if k
                ]
            }
        )
    ...
    def get_inbox(self):
        ...
        if r.status_code == 200:
            return [
                Letter(self._email, _letter, self.__get_xsrf_token(), self._s)
                for _letter in r.json()["messageData"]
                if "ADS" not in _letter["messageID"]
            ]
        return []
